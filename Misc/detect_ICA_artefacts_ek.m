...
temremove= unique([find(abs(Out.temcor(1,:))>0.3), find(abs(Out.temcor(2,:))>0.3)]);  %AP to remove third (ECG) channel
...
sparemove= unique([find(abs(Out.spacor(1,:))>0.7), find(abs(Out.spacor(2,:))>0.7)]); %AP remove third component  
