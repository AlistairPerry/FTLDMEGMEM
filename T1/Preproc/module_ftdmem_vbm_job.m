function module_ftdmem_vbm_job(group1_mrilist, group1_ages, group2_mrilist, group2_ages, group3_mrilist, group3_ages, preprocfolder)
...
addpath(genpath('/imaging/rowe/Michelle/FTLD_7T_CPSP/scripts')) 
...
%Re-change order of mri list
mrilist = [group1_mrilist; group2_mrilist; group3_mrilist];


%all_Controls = dir('/imaging/rowe/Michelle/AlistairIFG/B_Data/mC*denoised.nii');

%all_Patients = dir('/imaging/rowe/Michelle/AlistairIFG/B_Data/mP*denoised.nii');


ncons = size(group1_mrilist,1);

npats = size(group2_mrilist,1)+size(group3_mrilist,1);


% for i = 1:ncons
% 
% mrilist{i,1} = [all_Controls(i).folder '/' all_Controls(i).name];
% 
% end
% 
% 
% for i = 1:npats
% 
% mrilist{i+ncons,1} = [all_Patients(i).folder '/' all_Patients(i).name];
% 
% end
...
dodartel = 1;
doremdartel = 1;
%donormalise = 1; take this out as it gets confusing
donormalisec1 = 1;
doVBManalysis = 1;
...
if dosegment
    ...
    nrun = length(mrilist);
    %jobfile = {'/imaging/rowe/Michelle/FTLD_7T_CPSP/scripts/VBM_batch_segment.m'};
    jobfile = {'/imaging/rowe/Michelle/AlistairIFG/A_Scripts/VBM_batch_segment_0001.m'}; %0001 bias reg
...
if docalculatetiv
    ...
    nrun = 1;
    jobfile = {'/imaging/rowe/Michelle/FTLD_7T_CPSP/scripts/VBM_batch_TIV.m'};
    jobs = repmat(jobfile, 1, nrun);
    inputs = cell(2, nrun);
    ...
    mkdir([preprocfolder filesep ''])
    inputs{2,1} = [preprocfolder filesep 'volumes_VBM.csv'];
...
%Find cohort with smallest N, which would be the bv's (list 2)

%ncons = size(group1_mrilist, 1);

nbvs = size(group2_mrilist, 1);

npsps = size(group3_mrilist, 1);


randcons = randperm(ncons, nbvs);

randpsps = randperm(npsps, nbvs);

notusedcons = [1:ncons];
notusedpsps = [1:npsps];

notusedcons(randcons) = [];
notusedpsps(randpsps) = [];

core_imagepaths = [group1_mrilist(randcons); group2_mrilist; group3_mrilist(randpsps)];

if dodartel

nrun = 1; % enter the number of runs here
jobfile = {'/imaging/rowe/Michelle/FTLD_7T_CPSP/scripts/VBM_batch_dartel.m'};
jobs = repmat(jobfile, 1, nrun);
inputs = cell(2,1);
...
%Save
%save('/imaging/rowe/Michelle/AlistairIFG/A_Scripts/batch_dartel.mat','jobs');
save('/imaging/rowe/Michelle/AlistairIFG/A_Scripts/inputs_dartel.mat','inputs');
...
%AP add
remaining_control_imagepaths = group1_mrilist(notusedcons);
remaining_psp_imagepaths = group3_mrilist(notusedpsps);

 
nrun = 1; % enter the number of runs here
jobfile = {'/imaging/rowe/Michelle/FTLD_7T_CPSP/scripts/VBM_batch_templated_dartel.m'};

jobs = repmat(jobfile, 1, nrun);
inputs = cell(8, nrun);
inputs{1,1} = cell(length(remaining_control_imagepaths)+length(remaining_psp_imagepaths),1);
inputs{2,1} = cell(length(remaining_control_imagepaths)+length(remaining_psp_imagepaths),1);
...
split_stem = regexp(remaining_psp_imagepaths, '/', 'split');

for i = 1:length(remaining_psp_imagepaths)
    inputs{1,1}(i+ind) = cellstr(['/' fullfile(split_stem{i}{1:end-1}) '/rc1' split_stem{i}{end}]);
    inputs{2,1}(i+ind) = cellstr(['/' fullfile(split_stem{i}{1:end-1}) '/rc2' split_stem{i}{end}]);
end
...
%Save
save('/imaging/rowe/Michelle/AlistairIFG/A_Scripts/batch_remdartel.mat','jobs');
save('/imaging/rowe/Michelle/AlistairIFG/A_Scripts/inputs_remdartel.mat','inputs');
...
nrun = length(mrilist);
jobfile = {'/imaging/rowe/Michelle/FTLD_7T_CPSP/scripts/VBM_batch_normalise.m'};
jobs = repmat(jobfile, 1, nrun);
inputs = cell(3, nrun);
...
if exist('tiv_filename','var')
    ...
    filename = [preprocfolder filesep 'volumes_VBM.csv'];
end
delimiter = ',';
startRow = 2;
endRow = inf;
...
nrun = 1;
jobfile = {'/imaging/rowe/Michelle/FTLD_7T_CPSP/scripts/VBM_batch_factorial_TIV_age.m'};
jobs = repmat(jobfile, 1, nrun);
inputs = cell(6, nrun);


stats_folder = {[preprocfolder filesep 'VBM_stats_cnvspats/factorial_full_group_vbm_TIVnormalised_agecovaried_unsmoothedmask']};


%Combine patients into one list - AP

patmrilist = [group2_mrilist; group3_mrilist];

split_stem_group2 = regexp(patmrilist, '/', 'split');
...
for crun = 1:nrun
    ...
    inputs{3, 1} = cell(length(patmrilist),1);
    for i = 1:length(patmrilist)
...
try
    ...
    tivstem = [preprocfolder filesep 'volumes_VBM'];
...
%Add patient ages into one - AP

pat_ages = cat(1, group2_ages, group3_ages);
inputs{5, 1} = [group1_ages; pat_ages];
inputs{6, 1} = {'control_majority_unsmoothed_mask_c1_thr0.05_cons0.8.img'};


%Huh whats this
if ~exist(char(inputs{6, 1}),'file')
    core_imagepaths = [patmrilist; group1_mrilist];
    split_stem_template = regexp(core_imagepaths, '/', 'split');
    path_to_template_6 = cellstr(['/' fullfile(split_stem_template{1}{1:end-1}) '/Template_6.nii']);
    %make_VBM_explicit_mask(group2_mrilist, path_to_template_6, 'control')
    make_VBM_explicit_mask(group1_mrilist, path_to_template_6, 'control') %AP to just change to patmrilist - confirm later
end
...
jobfile = {'/imaging/rowe/Michelle/FTLD_7T_CPSP/scripts/VBM_batch_estimate.m'};
jobs = repmat(jobfile, 1, nrun);

spm_jobman('run', jobs, inputs{:});

jobfile = {'/imaging/rowe/Michelle/AlistairIFG/A_Scripts/VBM_batch_contrast_basicgrp.m'};
jobs = repmat(jobfile, 1, nrun);
...
stats_folder = {[preprocfolder filesep 'VBM_stats_cnvsbv/factorial_full_group_vbm_TIVnormalised_agecovaried_unsmoothedmask']};


%Combine patients into one list - AP

patmrilist = [group2_mrilist];
...
inputs{4, 1} = tiv(1:size(group1_mrilist,1)+size(group2_mrilist,1),1);


%Add patient ages into one - AP

pat_ages = cat(1, group2_ages);
inputs{5, 1} = [group1_ages; pat_ages];
inputs{6, 1} = {'control_majority_unsmoothed_mask_c1_thr0.05_cons0.8.img'};
...
%Huh whats this
if ~exist(char(inputs{6, 1}),'file')
    core_imagepaths = [group1_mrilist; patmrilist];
    split_stem_template = regexp(core_imagepaths, '/', 'split');
    path_to_template_6 = cellstr(['/' fullfile(split_stem_template{1}{1:end-1}) '/Template_6.nii']);
    %make_VBM_explicit_mask(group2_mrilist, path_to_template_6, 'control')
    make_VBM_explicit_mask(patmrilist, path_to_template_6, 'control') %AP to just change to patmrilist - confirm later
end
...
jobfile = {'/imaging/rowe/Michelle/AlistairIFG/A_Scripts/VBM_batch_contrast_cnvsbv.m'};
jobs = repmat(jobfile, 1, nrun);
...
% spm('defaults', 'PET');
% jobfile = {'/imaging/rowe/Michelle/FTLD_7T_CPSP/scripts/VBM_batch_results.m'};
% jobs = repmat(jobfile, 1, nrun);
% 
% spm_jobman('run', jobs, inputs{:});
...
stats_folder = {[preprocfolder filesep 'VBM_stats_cnvspsp/factorial_full_group_vbm_TIVnormalised_agecovaried_unsmoothedmask']};


%Combine patients into one list - AP

patmrilist = [group3_mrilist];
...
TIVext = [tiv(1:size(group1_mrilist,1)); tiv(size(group1_mrilist,1)+size(group2_mrilist,1)+1:end)];
inputs{4, 1} = TIVext;


%Add patient ages into one - AP

pat_ages = cat(1, group3_ages);
inputs{5, 1} = [group1_ages; pat_ages];
inputs{6, 1} = {'control_majority_unsmoothed_mask_c1_thr0.05_cons0.8.img'};
...
jobfile = {'/imaging/rowe/Michelle/AlistairIFG/A_Scripts/VBM_batch_contrast_cnvspsp.m'};
jobs = repmat(jobfile, 1, nrun);
...
stats_folder = {[preprocfolder filesep 'VBM_stats_bvvspsp/factorial_full_group_vbm_TIVnormalised_agecovaried_unsmoothedmask']};


%Combine patients into one list - AP

split_stem_group2 = regexp(group3_mrilist, '/', 'split');
split_stem_group1 = regexp(group2_mrilist, '/', 'split');

inputs{1, 1} = stats_folder;

for crun = 1:nrun
    inputs{2, 1} = cell(length(group2_mrilist),1);
    for i = 1:length(group2_mrilist)
        inputs{2,crun}(i) = cellstr(['/' fullfile(split_stem_group1{i}{1:end-1}) '/smwc1' split_stem_group1{i}{end}]);
    end
    inputs{3, 1} = cell(length(group3_mrilist),1);
    for i = 1:length(group3_mrilist)
        inputs{3,crun}(i) = cellstr(['/' fullfile(split_stem_group2{i}{1:end-1}) '/smwc1' split_stem_group2{i}{end}]);
    end
end
...
TIVext = tiv(size(group1_mrilist,1)+1:end);
inputs{4, 1} = TIVext;
...
%pat_ages = cat(1, group3_ages);
inputs{5, 1} = [group2_ages; group3_ages];
inputs{6, 1} = {'control_majority_unsmoothed_mask_c1_thr0.05_cons0.8.img'};


%Huh whats this
if ~exist(char(inputs{6, 1}),'file')
    core_imagepaths = [group2_mrilist; group3_mrilist];
    split_stem_template = regexp(core_imagepaths, '/', 'split');
    path_to_template_6 = cellstr(['/' fullfile(split_stem_template{1}{1:end-1}) '/Template_6.nii']);
    %make_VBM_explicit_mask(group2_mrilist, path_to_template_6, 'control')
    make_VBM_explicit_mask(group3_mrilist, path_to_template_6, 'control') %AP to just change to patmrilist - confirm later
end
...
jobfile = {'/imaging/rowe/Michelle/AlistairIFG/A_Scripts/VBM_batch_contrast_bvvspsp.m'};
jobs = repmat(jobfile, 1, nrun);
...
% jobfile = {'/imaging/rowe/Michelle/big7T/scripts/VBM_batch_factorial_TIV_age_singlesubj.m'};
% jobs = repmat(jobfile, 1, 1);
% inputs = cell(6, nrun);
% 
...
% 
% for crun = 1:nrun
%     inputs{1, crun} = {[preprocfolder filesep 'VBM_stats/factorial_single_subject/patient_' num2str(crun)]};
...
% end
% 
% jobfile = {'/imaging/rowe/Michelle/big7T/scripts/VBM_batch_estimate.m'};
% jobs = repmat(jobfile, 1, 1);
% 
% parfor crun = 1:nrun
%     spm('defaults', 'PET');
...
% end
% 
% jobfile = {'/imaging/rowe/Michelle/big7T/scripts/VBM_batch_contrast.m'};
% jobs = repmat(jobfile, 1, 1);
% 
% parfor crun = 1:nrun
%     spm('defaults', 'PET');
...
% end
% 
% jobfile = {'/imaging/rowe/Michelle/big7T/scripts/VBM_batch_results.m'};
% jobs = repmat(jobfile, 1, 1);
% 
% parfor crun = 1:nrun
%     spm('defaults', 'PET');
...
% spm_jobman('initcfg')
% %parfor crun = 1:nrun
% for crun = 1:nrun %Fails for Michelle in parallel, I don't know why, but doesn't take long serially, so just do that.
%     %     spm('defaults', 'PET');
%     %     spm_jobman('initcfg')
%     try
%         spm_jobman('run', jobs, inputs{:,crun});
...
% 
% %parfor crun = 1:nrun
% for crun = 1:nrun %Same as with the grey matter above, does not run. See if changing it works.
%     %spm('defaults', 'PET');
%     %spm_jobman('initcfg')
%     try
%         spm_jobman('run', jobs, inputs{:,crun});
...
% inputs = cell(6, nrun);
% 
% stats_folder = {[preprocfolder filesep 'VBM_stats/WM_factorial_full_group_vbm_TIVnormalised_agecovaried_unsmoothedmask']};
% split_stem_group2 = regexp(group2_mrilist, '/', 'split');
% split_stem_group1 = regexp(group1_mrilist, '/', 'split');
% 
% inputs{1, 1} = stats_folder;
...
%     inputs{4, 1} = tiv;
% catch
%     tivstem = [preprocfolder filesep 'volumes_VBM.csv'];
%     filename = [tivstem '.csv'];
%     delimiter = ',';
%     startRow = 2;
%     endRow = inf;
...
% spm_jobman('run', jobs, inputs{:});
% 
% jobfile = {'/imaging/rowe/Michelle/FTLD_7T_CPSP/scripts/VBM_batch_contrast.m'};
% jobs = repmat(jobfile, 1, nrun);
% 
% spm_jobman('run', jobs, inputs{:});
% 
