...
%Differs from original in that it goes off ssst file
...
%yes cannot epoch
%subjects = {'p3', 'p4','p5','p6','p7','p8','p9'}; %ran again because of aforementioned subject session

%Accidentally ran preproc on TGB controls - so re do all cons here
% subjects = {'c1','c10','c11','c12','c13','c14','c15','c16','c2','c21','c22','c23','c24','c3','c4','c5','c6','c7','c8','c9'};
% 
% 
% %Those which didn't go through maxfilter first time
% % subjects = {'c14', 'c16', 'p1', 'p11', 'p15', 'p19', 'p2', 'p23', 'p24', 'p4'}; 
% 

%Full below - 0604 - but w/out p3 as there was issues above with it
subjects = {'c1','c10','c11','c12','c13','c14','c15','c16','c2','c21','c22','c23','c24','c3','c4','c5','c6','c7','c8','c9','p1','p10','p11','p12','p13','p14','p15','p16','p17','p18','p19','p2','p21','p22','p23','p24','p25','p4','p5','p6','p7','p8','p9'};
sessions = {'s1', 's2'};


%Full patients below - 1304 - and w/out p3 as there was issues above with it
%subjects = {'p1','p10','p11','p12','p13','p14','p15','p16','p17','p18','p19','p2','p21','p22','p23','p24','p25','p4','p5','p6','p7','p8','p9'};
%sessions = {'s1', 's2'};
...
% 050421/sensor issues
%subjects = {'c8'};


%p12/13 - test sensor labelling changes
%subjects = {'p12'};
%sessions = {'s1','s2'};

%test 5 pla patients
%subjects = {'p1', 'p6', 'p7', 'p9', 'p12', 'p13'};
%sessions = {'s1'};
    ...
    for sub = 1:length(subjects)
        ...
        for sess = 1:length(sessions)
            ...
            if isfile(fullfile([maxfiltdir,'/',subjects{sub},'/', sessions{sess}, '/', subjects{sub} '_' sessions{sess} '_' origfname]))
                ...
                basefile = [subjects{sub} '_' sessions{sess} '_rov123_ssst'];
...
if dodown
    ...
    for sub=1:length(subjects)
        ...
        for sess = 1:length(sessions)
            ...
            if isfile(fullfile([ses_out_dir '/' 'S_' subjects{sub} '_' sessions{sess} '_rov123_ssst.mat']))
                
                
                basefile = ['S_' subjects{sub} '_' sessions{sess} '_rov123_ssst'];
...
if dofilter
    ...
    for sub=1:length(subjects)
        ...
        for sess = 1:length(sessions)
            ...
            if isfile(fullfile([ses_out_dir '/' 'dS_' subjects{sub} '_' sessions{sess} '_rov123_ssst.mat']))
                
                
                basefile = ['dS_' subjects{sub} '_' sessions{sess} '_rov123_ssst'];
...
if donotch
    ...
    for sub=1:length(subjects)
        ...
        for sess = 1:length(sessions)
            ...
            if isfile(fullfile([ses_out_dir '/' 'ffdS_' subjects{sub} '_' sessions{sess} '_rov123_ssst.mat']))
                
                
                basefile = ['ffdS_' subjects{sub} '_' sessions{sess} '_rov123_ssst'];
...
if doartefact % uses Oxford's OSL, works much better than SPM's.
    ...
    for sub=1:length(subjects)
        ...
        for sess = 1:length(sessions)
            ...
            if isfile(fullfile([ses_out_dir '/' 'ffffdS_' subjects{sub} '_' sessions{sess} '_rov123_ssst.mat']))
                
                
                basefile = ['ffffdS_' subjects{sub} '_' sessions{sess} '_rov123_ssst'];
...
if doica
    ...
    for sub=1:length(subjects)
        ...
        for sess = 1:length(sessions)
            ...
            if isfile(fullfile([ses_out_dir '/' 'affffdS_' subjects{sub} '_' sessions{sess} '_rov123_ssst.mat']))
                
                
                submaticacount = submaticacount+1; %AP add
                
                
                basefile = ['affffdS_' subjects{sub} '_' sessions{sess} '_rov123_ssst'];
                ...
                icafile = fullfile(ses_out_dir,sprintf('%s.mat',['M' basefile '.mat']));
    ...
    for sub=1:length(subjects)
        ...
        for sess = 1:length(sessions)
            ...
            if isfile(fullfile([ses_out_dir '/' 'affffdS_' subjects{sub} '_' sessions{sess} '_rov123_ssst.mat']))
                ...
                submaticacount = submaticacount + 1;
    ...
    for sub=1:length(subjects)
        ...
        for sess = 1:length(sessions)
            ...
            if isfile(fullfile([ses_out_dir '/' 'MaffffdS_' subjects{sub} '_' sessions{sess} '_rov123_ssst.mat']))
                ...
                for m=1:length(mod)
                    ...
                    load([ses_out_dir '/' 'affffdS' '_' subjects{sub} '_' sessions{sess} '_rov123_ssst_' mod{m} '_ica.mat']); %bit confused here as its pointing to pre-ICA file
...
if doepoch
    ...
    for sub=1:length(subjects)
        ...
        if strcmp(subjects{sub}, 'p3') %do this because p3 has s2 has epoching issues
            ...
            if isfile(fullfile([ses_out_dir '/' 'MaffffdS_' subjects{sub} '_' session '_rov123_ssst.mat']))
                
                
                basefile = ['MaffffdS_' subjects{sub} '_' session '_rov123_ssst'];
            ...
            for sess = 1:length(sessions)
                ...
                if isfile(fullfile([ses_out_dir '/' 'MaffffdS_' subjects{sub} '_' sessions{sess} '_rov123_ssst.mat']))
                    ...
                    basefile = ['MaffffdS_' subjects{sub} '_' sessions{sess} '_rov123_ssst'];
...
if doartefact2 % EK
    ...
    for sub=1:length(subjects)
        ...
        for sess = 1:length(sessions)
            ...
            if isfile(fullfile([ses_out_dir '/' 'eMaffffdS_' subjects{sub} '_' sessions{sess} '_rov123_ssst.mat']))
                
                
                basefile = ['eMaffffdS_' subjects{sub} '_' sessions{sess} '_rov123_ssst'];
...
if doaverage %EK
    ...
    parfor sub=1:length(subjects)
        ...
        for sess = 1:length(sessions)
            ...
            if isfile(fullfile([ses_out_dir '/' 'raeMaffffdS_' subjects{sub} '_' sessions{sess} '_rov123_ssst.mat']))
                
                
                basefile = ['raeMaffffdS_' subjects{sub} '_' sessions{sess} '_rov123_ssst'];
                
                
                D = spm_eeg_load([ses_out_dir,'/', 'raeMaffffdS' '_' subjects{sub} '_' sessions{sess} '_' 'rov123_ssst.mat']);
                
                S   = []; S.D = D;
                S.robust.savew = 0;
                S.robust.bycondition = 1;
                S.robust.ks = 3;
                %S.robust.removebad = 1; won't like this, as bad trials
                %aren't marked
                D = spm_eeg_average(S);
    ...
    for sub=1:length(subjects)
        ...
        for sess = 1:length(sessions)
            ...
            if isfile(fullfile([ses_out_dir '/' 'mraeMaffffdS_' subjects{sub} '_' sessions{sess} '_rov123_ssst.mat']))
                
                
                basefile = ['mraeMaffffdS_' subjects{sub} '_' sessions{sess} '_rov123_ssst'];
                ...
                S.D = [ses_out_dir,'/', 'mraeMaffffdS' '_' subjects{sub} '_' sessions{sess} '_' 'rov123_ssst.mat'];
                S.type = 'butterworth';
                S.order = 5;
                S.band = 'low';
                S.freq = low;
